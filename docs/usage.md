...
```bash
nextflow run famosab/vcftomat --input ./samplesheet.csv --outdir ./results --genome GATK.GRCh38 -profile docker
```
...
```bash
nextflow run famosab/vcftomat -profile docker -params-file params.yaml
```
...
```bash
nextflow pull famosab/vcftomat
```
